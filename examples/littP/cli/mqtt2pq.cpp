...
public:
  ...
  void update(uint8_t id, uint16_t value) {
    ...
    if (!idp)
      idp = new DataIDWithMeta(id, RWSpec::ReadWrite, "unknown", IDType::u16, "", this);
    idp->value = value;
  }
  ...
  virtual bool on_read(uint8_t id, uint16_t value) override {
    if (outp && id == 0) {
    ...
    return Application::on_read(id, value);
  }
...
static const char *mosq_topic = "littP/#";
